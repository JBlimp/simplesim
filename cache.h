...
/* junsuk: add for mshr */
#define MAX_MSHR_ENTRY 16

struct mshr_entry_t {
	md_addr_t block_addr;                // 요청 블럭 주소
	enum mem_cmd cmd;              // 명령(Read/Write)
	int bsize;                     // 블록 크기
	int ready;                     // 준비 여부
	struct mshr_entry_t *next;     // 다음 엔트리 포인터 (linked list)
	tick_t timestamp;				   // 등록된 시간
};

struct mshr_t {
	struct mshr_entry_t *head;     // MSHR 리스트의 head (linked list)
	int num_entries;               // 현재 MSHR에 저장된 엔트리 수
};
...
// MSHR에 엔트리 추가
int mshr_add(struct mshr_t *mshr, md_addr_t addr, enum mem_cmd cmd, int bsize, tick_t now);

// MSHR에서 엔트리 검색 (병합 여부 확인용)
struct mshr_entry_t *mshr_lookup(struct mshr_t *mshr, md_addr_t addr);
