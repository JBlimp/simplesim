...
dl2_access_fn(enum mem_cmd cmd,		/* access cmd, Read or Write */
	      ...
	      tick_t now)		/* time of access */
{
	/* junsuk: if already exist in mshr */
	if (mshr_lookup(&mshr, baddr))
	{
		return 0;
	} else
	{
		/* junsuk: add to mshr
		 * FIXME: 가득 찼을때의 동작 */
		mshr_add(&mshr, baddr, cmd, bsize, sim_cycle);
...
sim_init(void)
{
  ...
  /* junsuk: allocate and initialize mshr */
  mshr_init(&mshr);
}
...
#include "machine.def"
  ...
  regs.regs_PC = regs.regs_PC - sizeof(md_inst_t);
	...
	/* junsuk: current mshr pointer */
	struct mshr_entry_t* current;
  ...
  for (;;)
	...
	panic("LSQ_head/LSQ_tail wedged");
      ...
      ptrace_newcycle(sim_cycle);
  	  ...
  	  /* junsuk: check mshr */
  	  current = mshr.head;
  	mshr_update_ready(&mshr, sim_cycle, mem_access_latency(current->bsize));
  	  while (current)
  	  {
	  	  if (current->ready)
	  	  {
		  	  unsigned int latency = mem_access_latency(current->bsize);
	  	  	sim_cycle += latency;
	  	  	md_addr_t baddr = current->block_addr;
	  	  	current = current->next;
	  	  	mshr_remove(&mshr, baddr);
	  	  	continue;
	  	  }
  	  	current = current->next;
  	  }
