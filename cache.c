...
/* junsuk: add for mshr */
void mshr_init(struct mshr_t *mshr) {
    mshr->head = NULL;
    mshr->num_entries = 0;
}

int mshr_add(struct mshr_t *mshr, md_addr_t block_addr, enum mem_cmd cmd, int bsize, tick_t now) {
    if (mshr->num_entries >= MAX_MSHR_ENTRY) {
        return -1;  // MSHR가 꽉 참
    }

    // 새로운 엔트리 생성
    struct mshr_entry_t *new_entry = (struct mshr_entry_t *)malloc(sizeof(struct mshr_entry_t));
    new_entry->block_addr = block_addr;
    new_entry->cmd = cmd;
    new_entry->bsize = bsize;
    new_entry->ready = 0;
    new_entry->next = mshr->head;
...
void mshr_update_ready(struct mshr_t *mshr, tick_t now, unsigned int mem_lat)
{
    struct mshr_entry_t *current = mshr->head;
    while (current) {
        if (now - current->timestamp >= mem_lat) {
            current->ready = 1;
        }
...
struct mshr_entry_t *mshr_lookup(struct mshr_t *mshr, md_addr_t block_addr) {
    struct mshr_entry_t *current = mshr->head;
    while (current) {
...
void mshr_remove(struct mshr_t *mshr, md_addr_t addr) {
    struct mshr_entry_t *prev = NULL;
    struct mshr_entry_t *current = mshr->head;

    while (current) {
        if (current->block_addr == addr) {
            if (prev) {
                prev->next = current->next;
            } else {
                mshr->head = current->next;
            }
            free(current);
            mshr->num_entries--;
            return;
